...
class IuvsSamples(Samples):
    widgets = {}
    iuvs_choices = choices["Iuvs"]
    iuvs_choices["pure_or_mixture"] = [["pure", "Pure"], ["mixture", "Mixture"]]

    type_choices = tupleConvert(iuvs_choices["type_choices"])
    type = models.CharField(max_length=255, blank=True, choices=type_choices)

    # Adherent or Suspended
    adherent_or_suspended_choices = tupleConvert(iuvs_choices["adherent_or_suspended"])
    adherent_or_suspended = models.CharField(max_length=64, blank=True, choices=adherent_or_suspended_choices)

    # Bio Safety Level
    bio_safety_choices = tupleConvert(iuvs_choices["bio_safety"])
    ...
    # Chemical Hazard
    chemical_hazard_choices = tupleConvert(iuvs_choices["chemical_hazard"])
    ...
    # Form
    powder_form_choices = tupleConvert(iuvs_choices["powder_form"])
    ...
    # Pure or Mixture
    pure_or_mixture_choices = tupleConvert(iuvs_choices["pure_or_mixture"])
    pure_or_mixture = models.CharField(max_length=64, blank=True, choices=pure_or_mixture_choices)

    # Self-standing or Free-standing
    slice_standing_choices = tupleConvert(iuvs_choices["slice_standing"])
    ...
    class Meta:
        db_table= 'iuvs_samples'.lower()
        app_label = 'PRP_CDM_app'

class IuvsMetadata(models.Model):
