...
class Biosafety_FacilitySamples(Samples):
    widgets = {}
    biosafety_facility_choices = choices["Biosafety_Facility"]
    type_choices = tupleConvert(biosafety_facility_choices["type_choices"])
    type = models.CharField(max_length=255, blank=True)
    widgets["type"] = MultiChoicheAndOtherWidget(choices=type_choices)
    submitted_at = models.DateTimeField(default=timezone.now)

    # Bio Safety Level
    bio_safety_choices = tupleConvert(biosafety_facility_choices["bio_safety"])
    ...
    class Meta:
        db_table= 'biosafety_facility_samples'.lower()
